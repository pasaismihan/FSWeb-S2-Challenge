...
function cumlelereDonustur(dizi, ayrac) {
  let yeniArr = dizi.map((item) => item.join(ayrac));
...
function paragrafOlustur(dizi, callKur, callDonustur) {
  let yeniDizi = callDonustur(dizi, " ");
...
function emojileriDonustur(mesaj, emojiObj) {
  for (let key in emojiObj) {
    mesaj = mesaj.replaceAll(key.toUpperCase(), emojiObj[key]);
